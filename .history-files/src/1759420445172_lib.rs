...
lazy_static! {
    static ref circleclick: Mutex<bool> = Mutex::new(false);
}
lazy_static! {
...
lazy_static! {
    static ref drawclick: Mutex<bool> = Mutex::new(false);
}
lazy_static! {
...
pub fn start() -> Result<(), JsValue> {
...

    {
        ...
        let mouse_down = Closure::wrap(Box::new(move |event: MouseEvent| {
            ...
            let mut num3 = circleclick.lock().unwrap();
            let mut num2 = mouseclick.lock().unwrap();
            if (*num == true|| *num3 == true) {
                *num2 = true;
                console::log_2(&JsValue::from_bool(*num), &JsValue::from_bool(*num2));
            } else {
                *num2 = true;
                context.begin_path();
                context.move_to(new_x as f64, new_y as f64);
    ...
    }
    {
        ...
        let circle_down = Closure::wrap(Box::new(move |event: MouseEvent| {
            let mut num = circleclick.lock().unwrap();
        ...
        circle.add_event_listener_with_callback("click", circle_down.as_ref().unchecked_ref())?;
    ...
    }
    {
        let click_down = Closure::wrap(Box::new(move |event: MouseEvent| {
            ...
            let mut num1 = drawclick.lock().unwrap();
    ...
    }
    {
        ...
        let click_down = Closure::wrap(Box::new(move |event: MouseEvent| {
            ...
            let mut circle = circleclick.lock().unwrap();
            let mut mouse = mouseclick.lock().unwrap();
            let mut arrownum = arrowclick.lock().unwrap();
             let mut drawnum = drawclick.lock().unwrap();
            let mut lastnum=*num;
            ...
            if *arrownum==true && *mouse ==true {
             ...
             if *drawnum==true && *mouse ==true {
                console::log_1(&"TRYING ARROW MOUSE MOVE".into());
                context.line_to((new_x) as f64, (new_y) as f64);
                context.stroke();
            }   
            if *circle==true && *mouse ==true {
                context.begin_path();
                context.ellipse(*num as f64 , *num1 as f64,  (new_x-*num)  as f64, (new_y-*num1)  as f64,90 as f64, 0 as f64, 360 as f64);
                context.stroke();
            }
    ...
    }
    {
        ...
        let mouse_up = Closure::wrap(Box::new(move |event: MouseEvent| {
            ...
            *num1 = 0.0;
            ...
            //  context.stroke_rect(event.offset_x() as f64, event.offset_y() as f64, 10 as f64, 30 as f64);
            let mut button = buttonclick.lock().unwrap();
             let mut circle = buttonclick.lock().unwrap();
            let mut mouse = mouseclick.lock().unwrap();
            if (*button == true && *mouse == true) {
                *mouse = false;
                *button = false;
