...
use std::rc::Rc;
use std::f64;
use web_sys::{Event, HtmlCanvasElement, MouseEvent, WebGlProgram, WebGlRenderingContext,KeyboardEvent ,WebGlShader,CanvasRenderingContext2d,console};
...
pub fn start() -> Result<(), JsValue> {
    ...
    
    {
        ...
        let mouse_down=Closure::wrap(Box::new(move |event: MouseEvent| {
            ...
            context.stroke_text("Hello Rust", event.offset_x() as f64, event.offset_y() as f64);
        ...
        let keydown_down=Closure::wrap(Box::new(move |event: KeyboardEvent | {
             console::log_1(&"Hello from mouse event 2".into());
            let new_x = event.offset_x() as f64;
            let new_y = event.offset_y() as f64;
            // context.set_stroke_style_str("blue");
        ...
        console::log_2(&JsValue::from_f64(new_x), &JsValue::from_f64(new_y));
          ...
          canvas
            .add_event_listener_with_callback("keydown", keydown_down.as_ref().unchecked_ref())?;
