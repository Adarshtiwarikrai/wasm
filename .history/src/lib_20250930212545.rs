...
pub fn start() -> Result<(), JsValue> {
    ...
    }
    {
        let click_down = Closure::wrap(Box::new(move |event: MouseEvent| {
             let mut num=buttonclick.lock().unwrap();
             *num=!*num;
             console::log_1(&JsValue::from_bool(*num));
        }) as Box<dyn FnMut(_)>);
    ...
    }
    {
        ...
        let click_down = Closure::wrap(Box::new(move |event: MouseEvent| {
            ...
            if(*button==true && *mouse==true){
               context.clear_rect(*num as f64, *num1  as f64, (*lasty-*num) as f64, (*lasty-*num1)as f64);
                context.set_stroke_style_str("blue");
                context.stroke_rect(*num as f64, *num1  as f64, (new_x-*num) as f64, (new_y-*num1)as f64);
        ...
        let mouse_up = Closure::wrap(Box::new(move |event: MouseEvent| {
            ...
            *num1=0.0;
            ...
            if(*button==true && *mouse==true){
                ...
                *lastx=0.0;
                *lasty=0.0;
                console::log_2(&"mouse is up ",&JsValue::from_bool(*mouse));
            }
